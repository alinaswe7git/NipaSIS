...
//		if(substr($studentID, 0, 4) == DATE(Y) || substr($studentID, 1, 4) == DATE(Y)){
	...
	public function courseRegister($item) {
		...
		if ($run->num_rows == 0){
			echo'<div class="warningpopup">You cannot register your courses yet as you have no study set, please see your division coordinator to have that fixed!</div>';
			return FALSE;
		}
		...
		if ($run_exl->num_rows > 0){
			echo'<div class="warningpopup">You cannot register your courses because you are currently on <b>Exclude</b>, please see your school coordinator/ Academic office for advice!</div>';
			if ($this->core->role == 1000 || $this->core->role == 104 ) {
				echo'<div class="warningpopup"><a href="' . $this->core->conf['conf']['path'] . '/register/removeexclude/' . $userid . '"><b>Click here to remove EXCLUDE status</b></a></div>';
			}
			return FALSE;
		}
		...
		if($this->core->role == 104 ||  $this->core->role == 1000){
			...
			echo '<div class="successpopup">Registration is closed but you have access</div>'; 
		
		} else {
			while ($fetch = $runLT->fetch_assoc()){
				if($fetch['CourseRegEndDate'] < date('Y-m-d')){
					echo'<div class="warningpopup">Course Registration for current semester is closed!</div>';
					return FALSE;
				}	
			}
		...
		//End of late registration
			...
			*/
			...
			if($cc > 8 && $this->core->role != 104){
				echo'<div class="errorpopup">You have selected more than 8 courses. Please see division HoD to register more than 8 courses.</div>';
				return;
			}
...
/*
		...
		//End of late registration
			...
			while ($fetch = $run->fetch_assoc()) { 
				echo '<div class="errorpopup">You have already registered for this term.</div>';
				return FALSE;
			}
