...
class approval {
	...
	private function checkThreshold($userid, $num_courses = NULL, $period = NULL){
		...
		if($bill == 0){
			echo'<div class="errorpopup">Billing template for this semester has <b>not set</> for this program by Accounts Department, Please inform the Accounts Department to set it up.</div>';
			return FALSE;
		}
		...
		echo'<div class="warningpopup">STUDENT BALANCE: K'.$actual.' BILL 50%: K'.$threshold.'  BALANCE AFTER BILL: K'.$calc.'</div>';
	...
	public function manageApproval($item) {
		...
		$sql = "SELECT COUNT(DISTINCT `course-electives`.CourseID) as CT, FirstName, Surname, `basic-information`.ID as StudentID, `study`.Name
			...
			AND `course-electives`.PeriodID = $item
