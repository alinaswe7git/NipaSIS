...
class information {
	...
	public function getBalance($item){
			...
			//$output = file_get_contents("http://41.63.16.6:8081/AccPackService/balance/$item");
		...
		while ($row = $run->fetch_row()) {
			...
			while ($fetchw = $runo->fetch_assoc()) {
				...
				if($fetchw['Approved'] == 1){
					echo'<li>'.$fetchw['Name'].'  - '.$fetchw['CourseDescription'].'</i><br>';
				}else{
					echo'<font color="grey"><li >'.$fetchw['Name'].'  - '.$fetchw['CourseDescription'].' <b>(Not Approved)</b></i></font><br>';
				}
