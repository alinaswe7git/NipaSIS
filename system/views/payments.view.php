	...
	public function checkallPayments(){ 
		...
		while ($row = $run->fetch_row()) {
			...
			while($runx == TRUE){
				...
				$credit = $vals[$index["credit"][$i]]["value"];
				$debit = $vals[$index["debit"][$i]]["value"];
				$description = $vals[$index["description"][$i]]["value"];
				$reference = $vals[$index["reference"][$i]]["value"];
				$date = $vals[$index["txdate"][$i]]["value"];
				
	
	
				$payment[$i]["debit"] = $debit;
				$payment[$i]["credit"] = $credit;
				$payment[$i]["description"] = $description;
				$payment[$i]["reference"] = $reference;
				$payment[$i]["date"] = $date;
				
				if ($credit > 0){
					$payment[$i]["amount"] = $credit;
				}elseif ($debit > 0){
					$payment[$i]["amount"] = $debit;
				}else{
					$payment[$i]["amount"] = $credit-$debit;
				}
			...
			foreach($payment as $trans){
				...
				$sql = "INSERT INTO `paymets-trail` (`TDate`, `Type`, `Description`, `Amount`, `StudentID`, `TranID`) VALUES ('$date', '$type', '$description',$amount, $item, '$iid')";
